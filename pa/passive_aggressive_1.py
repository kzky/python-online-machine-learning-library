...
class PassiveAggressiveI(object):
    """
    Passive Aggressive-I algorithm: standard hinge loss PA.
    ...
    def _update(self, label, sample, margin):
        ...
        min_ = min(1/self.model["C"], (1-margin)/norm)
        w = self.model["w"] + label * min_ * sample
        self.model["w"] = w
    ...
    @classmethod
    def examplify(cls, fname, delimiter = " ", C = 1, n_scan = 100):
        """
        ...
        # learn
        st = time.time()
        model = PassiveAggressiveI(fname, delimiter, C, n_scan)
        model.learn()
        et = time.time()
...
if __name__ == '__main__':
    ...
    PassiveAggressiveI.examplify(fname, delimiter = " ", C = 1, n_scan = 100)
