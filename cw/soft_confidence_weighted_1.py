...
class SCWI(object):
    """
    Full matrix version of Soft Confidence-Weighted algorithm with L1-hinge loss.
    ...
    def _update(self, label, sample, m, v):
        ...
        phi_4 = self.model["phi_4"]
        zeta = self.model["zeta"]
        psi = self.model["psi"]
        C = self.model["C"]
        alpha = max(0, (-m*psi + np.sqrt(m**2 * phi_4/4 + v*phi_2*zeta) ) / (v*zeta))
        alpha = min(C, alpha)
    ...
    def examplify(cls, fname, delimiter = " ", eta = 0.1, C = 1, n_scan = 1):
        ...
        # learn
        model = SCWI(fname = fname, delimiter = delimiter, eta = eta, C = C, n_scan = n_scan)
        model.learn()
...
if __name__ == '__main__':
    ...
    SCWI.examplify(fname, delimiter = " ", eta = 0.9, C = 1, n_scan = 10)
