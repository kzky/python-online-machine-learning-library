...
class MSCWDIDiag(object):
    ...
    def __init__(self, fname, delimiter = " ", C = 1, eta = 0.9, n_scan = 10):
        """
    ...
    def _update(self, sample, y, r):
        ...
        # alpha
        first_term = -m*psi
        second_term = np.sqrt(np.power(m, 2)*np.power(phi_2, 2)/4 + phi_2*v_zeta)
    ...
    @classmethod
    def examplify(cls, fname, delimiter = " ", C = 1, eta = 0.9, n_scan = 1):
        """
        Example of how to use
        """
        # learn
        model = MSCWDIDiag(fname = fname, delimiter = delimiter, C = C, eta = eta, n_scan = n_scan)
        model.learn()
...
if __name__ == '__main__':
    ...
    MSCWDIDiag.examplify(fname, delimiter = " ", C = 1, eta = 0.9, n_scan = 10)
