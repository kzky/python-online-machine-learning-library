...
class Arow2(object):
    ...
    def _update(self, label, sample, margin):
        ...
        mu = self.model["mu"] + label * (1 - margin) * self.model["S"].dot(sample)/beta
        self.model["mu"] = mu

        # S
        outer_dot_sample = np.outer(sample, sample)
        S = self.model["S"] - self.model["S"].dot(outer_dot_sample).dot(self.model["S"])/beta
        self.model["S"] = S
    ...
    def examplify(cls, fname, delimiter = " ", C = 1, n_scan = 1):
        ...
        # learn
        model = Arow2(fname = fname, delimiter = delimiter, C = C, n_scan = n_scan)
        model.learn()
...
if __name__ == '__main__':
    ...
    Arow2.examplify(fname, delimiter = " ", C = 1, n_scan = 3)
